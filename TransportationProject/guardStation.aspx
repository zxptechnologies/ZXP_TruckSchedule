...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server" EnablePageMethods="true"></asp:ScriptManager>
     ...
     <script type="text/javascript">
         ...
         function onSuccess_checkIn(value, MSID, methodName) {
             ...
             onclick_OpenMoreDetails(StatusID, gMSID, false);
         ...
         function onSuccess_getNetWeightAndSpecificGravity(specGrav, netWeight, methodName) {
             ...
             if (netWeight == 0 || specGrav == 0) {
                 ...
                 $("#lblNoSamples").show();
                 if (netWeight == 0 && specGrav == 0) {
                     $("#lblNoSamples").html("Product weight cannot be calculated. </br>A sample with specific gravity has not been</br> approved and the net weight has not been calculated for this order.");
                     $("#lblSpecificGravityVal").hide();
                     $("#lblSpecificGravity").hide();
                     $("#btnCalcVol").hide();                     
                 }
                 else if (netWeight == 0) {
                     $("#lblNoSamples").html("Product weight cannot be calculated. </br>Net weight has not been calculated for this</br> order.");
                     $("#lblSpecificGravityVal").show();
                     $("#lblSpecificGravity").show();
                     $("#btnCalcVol").hide();
                 }
                 else if (specGrav == 0) {
                     $("#lblNoSamples").html("Product weight cannot be calculated. </br>A sample with specific gravity has not been</br> approved yet for this order.");
                     $("#lblSpecificGravityVal").hide();
                     $("#lblSpecificGravity").hide();
                     $("#btnCalcVol").hide();
                 }
    ...
    function onFail_verifyAndMove(value, ctx, methodName) {
        ...
        -------------------------------------------------------%>
         ...
         function onclick_OpenMoreDetails(currentStatus, MSID, isCheckOut) {
                 ...
                 columns:
                     [
                        ...
                        { headerText: "", key: "PRODCOUNT", dataType: "number", width: "0px", hidden: true },
                        {
                         ...
                         //{
                         //    headerText: "Weights", key: "WEIGHTS", dataType: "number", width: "75px",
                         ...
                         //{
                         //    headerText: "Send to...", key: "TOWAITINGAREA", dataType: "string", width: "85px",
                         ...
                         //},
                         //{
                         //    headerText: "Additional Details", key: "DRIVERINFO", dataType: "string", width: "85px", template: "{{if (checkNullOrUndefined(${CHECKIN}) == false)}} <div class ='ColumnContentExtend'><div class=\"tooltip\"> <input id='btnDriverInfo' type='button' value='Details' onclick='onClick_OpenDetailsDialogBox(${MSID});' class='ColumnContentExtend'/><span class=\"tooltiptext\">Driver Info, Files, Cab #s & GS Comments</span><\div></div> {{/if}}"
                         //},
                         {
                             headerText: "Details", key: "POPUPWINDOW", dataType: "string", width: "85px", template: "{{if (checkNullOrUndefined(${CHECKIN}) == false)}} <div class ='ColumnContentExtend'><div class=\"tooltip\"> <input id='btnPopUpDetails' type='button' value='More Details' onclick='onclick_OpenMoreDetails(${STATUS}, ${MSID}, false);' class='ColumnContentExtend'/><span class=\"tooltiptext\">More information</span><\div></div> {{/if}}"
                         },
    ...
    <div id ="MoreDetailsDialog">
       ...
       <%--  <input id='btnCloseMoreDetails' type='button' value='Close' onclick='onClick_CloseMoreDetails();'/> --%>
    </div>
