...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <h3>Turn manual input for weights on and off. </h3>
     ...
     <script type="text/javascript">
         ...
         function onSuccess_updateManualInputValue(value, isEnabled, methodName) {
             if (isEnabled == true) {
                 alert("Manual entry for weighting has been enabled.");
             }
             else {
                 alert("Manual entry for weighting has been disabled.");
             }
         ...
         var updateManualWeightValue = function () {
             if (document.getElementById('manualWeights').checked) {
                 //enable
                 PageMethods.updateManualInputValue(true, onSuccess_updateManualInputValue, onFail_updateManualInputValue, true);
             } else {
                 //disable
                 PageMethods.updateManualInputValue(false, onSuccess_updateManualInputValue, onFail_updateManualInputValue, false );
             }
         };
