...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function onclick_btnApproveBypassDialog() {
            ...
            if (canBypass == true) {
                if (hasComment == false) {
                    var sampleID = $("#bypassDialog").data("data-SampleID");
                    var MSID = $("#bypassDialog").data("data-MSID");
        ...
        function onclick_addCOFA(SampleID) {
            sessionStorage.setItem('sampleID', SampleID);
            location.href = 'COFAUpload.aspx';
        }

        function onclick_BypassCOFA(SampleID) {
            $("#commentDialog").val('');
            $("#txtLabAdminPassword").val('');
            $("#txtLabAdminUN").val('');
            var canBypass = $("#gridSamples").data("data-isLabAdmin");
            ...
            var PO = $("#gridSamples").igGrid("getCellValue", SampleID, "PO");
            var trailer = $("#gridSamples").igGrid("getCellValue", SampleID, "TRAILER");
            var products = $("#gridSamples").igGrid("getCellValue", SampleID, "PRODUCT");
            var bypassComment = $("#gridSamples").igGrid("getCellValue", SampleID, "BYPASSCOMMENT");

            $("#sampleIDDialog").html("Sample ID: " + SampleID);
            $("#bypassDialog").data("data-SampleID", SampleID);
            $("#MSIDDialog").html("MSID: " + MSID);
            $("#bypassDialog").data("data-MSID", MSID);
            $("#PODialog").html("PO: " + PO);
        ...
        function onSuccess_getCurrentLocationAndStatus(currentLocStatus, MSID, methodName) {
            ...
            if (canSample) {
                ...
                if (locLong == 'Guard Station' || locLong == 'Not On Site') {
                    alert("Sample cannot be taken. PO's location is " + locLong + " and sampling can not be done from " + locLong + ". Please update location and try again");
                }
                else {
        ...
        function onSuccess_undoApproveSample(value, MSID, methodName) {
         ...
         function onSuccess_getSampleGridData(value, ctx, methodName) {
             ...
             for (i = 0; i < value.length; i++) {
                 var bypasserName;
                 var isOpenInCMS;
                 if (checkNullOrUndefined(value[i][16]) == true) {
                     bypasserName = "";
                 }
                 else {
                     bypasserName = value[i][16] + " " + value[i][17];
                 }
                 ...
                 GLOBAL_SAMPLEGRID_DATA[i] = {
                     ...
                     "BYPASSER": bypasserName, "BYPASSCOMMENT": value[i][18], "GRAVITY": value[i][19], "isOpenInCMS": isOpenInCMS, "REJECTED": value[i][21], "PRODUCTDETAILS": value[i][22]
                 };
             }
         ...
         function onSuccess_getSampleGridDataByMSID(value, ctx, methodName) {
             ...
             for (i = 0; i < value.length; i++) {
                 ...
                 GLOBAL_SAMPLEGRID_DATA[i] = {
                     ...
                     "BYPASSCOMMENT": value[i][18], "GRAVITY": value[i][19], "REJECTED": value[i][21], "isOpenInCMS": isOpenInCMS, "PRODUCTDETAILS": value[i][22]
                 };
             }
             $("#gridSamples").igGrid("option", "dataSource", GLOBAL_SAMPLEGRID_DATA);
             $("#gridSamples").igGrid("dataBind");
             ...
             $("#gridSamples").igGrid({
                 ...
                 columns: [
                    ...
                    { headerText: "Product", key: "PRODUCTDETAILS", dataType: "string", width: "150px" },
                    {
                        ...
                        template: "{{if (checkNullOrUndefined(${STAKENTIME}))===true}}<div class ='ColumnContentExtend'><input id='btnSTaken' type='button' value='Sample Taken' onclick='GLOBAL_ISDIALOG_OPEN = true; onclick_sampleTaken(${MSID}, ${SAMPLEID});' class='ColumnContentExtend'></div>" +
                    ...
                    },
                    {
                        ...
                        template: "{{if (!checkNullOrUndefined(${STAKENTIME}) && checkNullOrUndefined(${SSENTTIME}))===true}}<div class ='ColumnContentExtend'><input id='btnSent' type='button' value='Sample Sent' onclick= 'GLOBAL_ISDIALOG_OPEN = true; onclick_sampleSent(${MSID},${SAMPLEID}, ${PO}, \"${PRODUCT}\");' class='ColumnContentExtend'></div>" +
                    ...
                    },
                    {
                        ...
                        template: "{{if (!checkNullOrUndefined(${STAKENTIME}) && !checkNullOrUndefined(${SSENTTIME}) && checkNullOrUndefined(${SRECVTIME}))===true}}<div class ='ColumnContentExtend'><input id='btnSReceived' type='button' value='Sample Received' onclick='GLOBAL_ISDIALOG_OPEN = true; onclick_sampleReceived(${MSID},${SAMPLEID}, ${PO}, \"${PRODUCT}\");' class='ColumnContentExtend'></div>" +
                    ...
                    {
                        headerText: "COFA", key: "FILEUPLOAD", dataType: "string", width: "100px", //<div style='background-color:lightcoral; width: 100%;'>
                    ...
                    {
                        headerText: "Bypass COFA", key: "BYPASSER", dataType: "string", width: "120px",
                        template: "{{if (checkNullOrUndefined(${BYPASSER}) === true  && checkNullOrUndefined(${FILENAME})) === true && (!checkNullOrUndefined(${SRECVTIME}))}}<div class ='ColumnContentExtend'><input id='btnBypassCOFA' type='button' value='Bypass COFA' onclick='GLOBAL_ISDIALOG_OPEN = true; onclick_BypassCOFA(${SAMPLEID});' class='ColumnContentExtend'></div>" +
                            "{{elseif (!checkNullOrUndefined(${BYPASSER}))}} Bypass approved by: ${BYPASSER} " +
                            "{{else}} <div></div> {{/if}}"
                    },
                    { headerText: "Bypass Comment (max. 250)", key: "BYPASSCOMMENT", dataType: "string", width: "225px" }
                 ],
                 //autofitLastColumn: true,
                 features: [
                     {
                     ...
                     },
                        {
                            ...
                            editRowStarting: function (evt, ui) {
                                var row = ui.owner.grid.findRecordByKey(ui.rowID);
                                var isTimeColBtnClicked = $("#gridSamples").data("data-disabledColsClick");
                                ...
                                if (isTimeColBtnClicked || isBtnClicked) { <%-- end editing if time column btns were clicked, continue edit mode only when other cells are clicked --%>
                            ...
                            editRowEnding: function (evt, ui) {
                                if (!ui.rowAdding) {
                                    ...
                                    if (ui.update == true && ui.rowAdding == false && ((evt.originalEvent.type == "click" && evt.originalEvent.currentTarget.innerText == "Done") || evt.keyCode == 13)) {
                                        PageMethods.updateCommentLotusIDAndGravity(ui.values.SAMPLEID, ui.values.COMMENTS, ui.values.TESTID, ui.values.GRAVITY, ui.values.BYPASSER, onSuccess_updateCommentLotusIDAndGravity, onFail_updateCommentLotusIDAndGravity);
                                    }
                                    else {
                                        return false;
                                    }
        ...
        function initMobileGrid() {
            $("#gridSamples").igGrid({
                ...
                columns: [
                   ...
                   { headerText: "Product", key: "PRODUCT", dataType: "string", width: "25%" },
        ...
        <table style="border: 0;">
            ...
            <tr>
                <td>
                    ...
                    <button type="button" id="bypassBtnCancel" onclick='onclick_btnCancelBypassDialog(); return false;'>Cancel</button>
                </td>
            </tr>
        </table>
    </div>
