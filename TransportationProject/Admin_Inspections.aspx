...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    function onSuccess_checkInspectionHeaderName(value, ctx, methodName) {
        ...
        var newInspectionName = $("#txtBoxInspectionName").val();
            ...
            if (isNewInspection == true) {
                if (value.length > 0) { <%--if inspection name exist, alert user and change border of text box to red --%>
                    alert("Another inspection is already named " + ctx + ". Please rename the inspection.");
        ...
        function onSuccess_checkInspectionHeaderNameSaveDialogForNewInspections(value, ctx, methodName) {
            var isNewInspection = $("#dwEditInspectionHeader").data("data-isNewInspection");
            var newInspectionName = $("#txtBoxInspectionName").val();
            ...
            if (value.length > 0) { <%--if inspection name exist, alert user and change border of text box to red --%>
                ...
                alert("Inspection will be created after its first question has been added.");
                PageMethods.getInspectionList(onSuccess_getInspectionList, onFail_getInspectionList, inspectionName);
            }
        }

        function onSuccess_checkInspectionHeaderNameSaveDialogForExistingInspection(value, ctx, methodName) {
            var isNewInspection = $("#dwEditInspectionHeader").data("data-isNewInspection");
            var newInspectionName = $("#txtBoxInspectionName").val();
            var isPartOfInspectionList = $("#dwEditInspectionHeader").data("data-isPartOfInspectionList");
            var OriginallyIs2Runner = $("#grid").data("data-is2Runner");
            var canContinueSaving = false;
            var checkBoxVal = $("#chBoxValidation").is(':checked');
            ...
            if (isPartOfInspectionList == true) {
                if (OriginallyIs2Runner == false && checkBoxVal == true) {
                    var c = confirm("You can not update this inspection to a validation inspection because it is already a part of an inspection list. You must delete this inspection and recreate it or remove the inspection from all inspection list before updating to a validation inspection. Would you like to save the other changes?");
                    if (c == true) {
                ...
                else if (OriginallyIs2Runner == true && checkBoxVal == false) {
                    var c = confirm("You can not remove the 2nd validation on this inspection because it is already a part of an inspection list. You must delete this inspection and recreate it or remove the inspection from all inspection list before removing the 2nd validation inspection. Would you like to save the other changes?");
                    if (c == true) {
                        canContinueSaving = true;
                        checkBoxVal = true;
                    }
        ...
        function onSuccess_checkIfLastTestAndDisable(value, testID, methodName) {
            if (value) {
                ...
                if (value == 1) {
                    ...
                    var c = confirm("The question you are about to delete is the last question in " + currentInspectionName + ". By deleting this question you will also delete " + currentInspectionName +
                                    ". These actions can not be undone. Would you like to continue deleting this question?");
                ...
                if (value > 1) {
                    ...
                    var c = confirm("Continue deleting question? This action can not be undone");
        ...
        function onSuccess_checkIfInspectionAddedToInspectionList(numberOfILInspectionIsApartOf, ctx, methodName) {
            if (numberOfILInspectionIsApartOf > 0) {
                $("#dwEditInspectionHeader").data("data-isPartOfInspectionList", true);
            }
            else {
                $("#dwEditInspectionHeader").data("data-isPartOfInspectionList", false);
            }
        ...
        function onFail_checkIfInspectionAddedToInspectionList(value, ctx, methodName) {
            sendtoErrorPage("Error in Admin_Inspections.aspx, onFail_checkIfInspectionAddedToInspectionList");
        }
        ...
        function onClick_editHeaderData() {
            var selectedInspectionHeaderID = $("#cboxInspectionList").igCombo("value");
            PageMethods.checkIfInspectionAddedToInspectionList(selectedInspectionHeaderID, onSuccess_checkIfInspectionAddedToInspectionList, onFail_checkIfInspectionAddedToInspectionList);
        ...
        function onClick_deleteHeaderData() {
            var c = confirm("Deleting an inspection will delete the inspection, the inspection from all list it appears in, and the deal breaker alerts associated to it. These actions can not be undone. Would you like to continue deleting this inspection?");
            if (c == true) {
                var currentInspectionID = $("#dwEditInspectionHeader").data("data-CurrentInspectionID");
                $("#tblInspectionListDetails").hide();
                PageMethods.disableInspection(currentInspectionID, onSuccess_disableInspection, onFail_disableInspection);
        ...
        function onClick_SaveInspectionHeaderEdits() {
            ...
            if (checkNullOrUndefined(inspectionName) == false) {
                if (isNewInspection == true) {
                    PageMethods.checkInspectionHeaderNameForNewInspection(inspectionName, onSuccess_checkInspectionHeaderNameSaveDialogForNewInspections, onFail_checkInspectionHeaderName, inspectionName);
                }
                else {
                    var InspectionHeaderID = $("#cboxInspectionList").igCombo("value");
                    PageMethods.checkInspectionHeaderNameOfExisting(inspectionName, InspectionHeaderID, onSuccess_checkInspectionHeaderNameSaveDialogForExistingInspection, onFail_checkInspectionHeaderName, inspectionName);
                }
            }
            else {
                alert("Inspection name can not be blank.");
            ...
            $("#grid").igGrid({
                ...
                columns:
                    [
                        ...
                        { headerText: "Test Text", key: "TESTTEXT", dataType: "string",width:"1400px" },
                            ...
                            if ((evt.originalEvent.type == "click" && evt.originalEvent.currentTarget.innerText == "Done") || evt.keyCode == 13) {
                                ...
                                for (var i = 0; i < GLOBAL_GRID_DATA.length; i++) {
                                    ...
                                    if (newTest == testingTest && (ui.rowID != GLOBAL_GRID_DATA[i].TESTID)) {
                                        alert('A question with that text already exist. Test text must be unique.');
                                        ui.keepEditing = true;
                                        return false;
                                    }
                                }
                                if (ui.update == true && ui.rowAdding == true) {//adding
                                    ...
                                    var dealBreakerCounter = -1; //needs to be set to -1 because will count the the one that was just changed
                                    var ds = $("#grid").igGrid("option", "dataSource");
                                    if (ui.oldValues.ISDEALBREAKER == true && ui.values.ISDEALBREAKER == false) {
                                        for (var i = 0; i < ds.length; i++) {
                                            if (ds[i].ISDEALBREAKER == true) {
                                                dealBreakerCounter++;
                                            }
                                        }
                                    }
                                    var inspectionID = $("#dwEditInspectionHeader").data("data-CurrentInspectionID");
                                    if (dealBreakerCounter == 0) {
                                        var c = confirm("You are removing the last deal breaker question of this inspection. Doing this will delete any deal break alerts associated with this inspection and can not be undone. Would you like to continue removing this deal breaker?");
                                        if (c == true) {
