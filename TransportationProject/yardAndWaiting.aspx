...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function onSuccess_checkIfCanUpdateTrailerNumber(canUpdateTrailer, trailerNumber, methodName) {
            var MSID = $("#yardGrid").data("data-MSID");
            if (canUpdateTrailer == false) {
                ...
                //callUpdatePrompt(MSID, "Can not update trailer number because there is another trailer with the same number in the facility. Update trailer number to (max 15 characters): ");
            }
            else {
                PageMethods.updateTrailerNumber(trailerNumber, MSID, onSuccess_updateTrailerNumber, onFail_updateTrailerNumber, MSID);
            }
        }
        ...
        function onSuccess_updateLocation_ViaButtons(value, MSID, methodName) {
            var selectedMSID = $("#cboxTruckList").igCombo("value");
            if (selectedMSID == MSID) {
                var newLocLong = $("#cboxLocations").data("data-location");
                $("#lblLocation").html('Current Location: ' + newLocLong);
                $("#lblStatus").html("Current Status: Waiting");
            }
            else {
                onclick_btncloseLocation();
            }
        ...
        function callUpdatePrompt(MSID, promptMsg)
        {
            ...
            if (checkNullOrUndefined(trailerNum)) {
                ...
                if (trailerNum.length > 15) {
                    ...
                    PageMethods.checkIfCanUpdateTrailerNumber(MSID, trailerNum, onSuccess_checkIfCanUpdateTrailerNumber, onFail_checkIfCanUpdateTrailerNumber, trailerNum);                    
                }
            }
        }
        ...
        function onSuccess_updateTrailerNumber(value, MSID, methodName)
        {
            showProgress();
        ...
        function onSuccess_verifyMove_ToWait(returnData, PKEY, methodName) {
            if (returnData.length > 0) {
                ...
                $("#cboxLocations").data("data-location", "WAIT");
                if (returnData[0] == "true") {
                    PageMethods.updateLocation(MSID, "WAIT", onSuccess_updateLocation_ViaButtons, onFail_updateLocation, MSID);
                }
                else {
                    var PO = $("#yardGrid").igGrid("getCellValue", PKEY, "PO");
        ...
        function onSuccess_verifyMove_ToYard(returnData, MSID, methodName) {
            if (returnData.length > 0) {
                if (returnData[0] == "true") {
                    $("#cboxLocations").data("data-location", "YARD");
                    PageMethods.updateLocation(MSID, "YARD", onSuccess_updateLocation_ViaButtons, onFail_updateLocation, MSID);
                }
                else {
        ...
        function checkCookieAndRedirect() {
            window.setInterval(function () {
                var myCookie = document.cookie.substring(document.cookie.lastIndexOf(".ASPXAUTH=") + 1);

                var currentPage = window.location.pathname.substring(window.location.pathname.lastIndexOf("/") + 1).replace(".aspx", "");

                if ((document.cookie == null || document.cookie == "" ) && currentPage != "Login") {
        ...
        $(function () {
            ...
            $(document).delegate("#yardGrid", "iggridcellclick", function (evt, ui) {

                if (ui.colKey === 'PRODDETAIL' || ui.colKey === 'MOVE' || ui.colKey === 'DROPPEDTIME' || ui.colKey === 'EMPTYTIME') {
                    $("#yardGrid").data("data-BUTTONClick", true);
                }
                else {
                    $("#yardGrid").data("data-BUTTONClick", false);
                }

            });
            $(document).delegate("#waitGrid", "iggridcellclick", function (evt, ui) {

                if (ui.colKey === 'PRODDETAIL' || ui.colKey === 'MOVE' || ui.colKey === 'EMPTYTIME') {
                    $("#waitGrid").data("data-BUTTONClick", true);
                }
                else {
                    $("#waitGrid").data("data-BUTTONClick", false);
                }
            ...
            $("#yardGrid").igGrid({
                ...
                columns:
                    [
                        ...
                        {
                            headerText: "Emptied", key: "EMPTYTIME", dataType: "object",
                            ...
                            "{{elseif (${isDROPTRAILER} === true && checkNullOrUndefined(${EMPTYTIME}) === true)}}<div class ='ColumnContentExtend'> <input id='btnEmpty' type='button' value='Empty the Truck' onclick='onClick_emptyTheTrailer(${MSID});' class='ColumnContentExtend'> </div> " +
                    ...
                    },
                    {
                    ...
                    },
                     {
                         ...
                         editRowStarting: function (evt, ui) {
                             ...
                             var isUploadBtnClicked = $("#yardGrid").data("data-BUTTONClick");
                 ...
                 features: [
                    {
                     ...
                     },
                     {
                     ...
                     },
                      {
                          ...
                          editRowStarting: function (evt, ui) {
                              <%--stop editing if button was clicked instead of cell entry for editing--%>
                              var isUploadBtnClicked = $("#waitGrid").data("data-BUTTONClick");
                              if (isUploadBtnClicked) {
                                  $("#waitGrid").data("data-BUTTONClick", false);
                                  return false;
                              }
    ...
    <div id="yardGridWrapper">
        ...
        <table id="yardGrid" class="scrollGridClass"></table>
    </div>
    <div id="waitGridWrapper">
        <h2>Trailers in waiting area</h2>
        <table id="waitGrid" class="scrollGridClass"></table>
    </div>
